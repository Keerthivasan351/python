...
def update_clock():
    ...
    for i in range(12):
        ...
        for i in range(60):
            ...
            x1 = WIDTH/2 + 0.08 * WIDTH/2 * math.cos(angle)
            y1 = HEIGHT/2 + 0.08 * HEIGHT/2 * math.sin(angle)
            x2 = WIDTH/2 + 0.09 * WIDTH/2 * math.cos(angle) 
            y2 = HEIGHT/2 + 0.09 * HEIGHT/2 * math.sin(angle)
            if i % 5 == 0:
        ...
        second_x=WIDTH/2 + 0.6 * WIDTH/2 *math.cos(minute_angle)
        second_y=HEIGHT/2 + 0.6 * HEIGHT/2 *math.sin(minute_angle)
